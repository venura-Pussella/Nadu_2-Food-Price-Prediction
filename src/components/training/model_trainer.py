...
def save_train_test_data_model_evaluation(train_x, train_y, test_x, test_y, train_x_dates, train_y_dates, test_x_dates, test_y_dates, config):
    try:
        # Flatten 3D arrays into 2D for saving train/test feature data
        train_x_df = pd.DataFrame(train_x.reshape(train_x.shape[0], -1))
        test_x_df = pd.DataFrame(test_x.reshape(test_x.shape[0], -1))

        # Save labels (train_y and test_y)
        train_y_df = pd.DataFrame(train_y, columns=[f"label_day_{i+1}" for i in range(train_y.shape[1])])
        test_y_df = pd.DataFrame(test_y, columns=[f"label_day_{i+1}" for i in range(test_y.shape[1])])

        # Save corresponding dates for train/test feature data
        train_x_dates_df = pd.DataFrame(train_x_dates, columns=[f"seq_day_{i+1}" for i in range(len(train_x_dates[0]))])
        test_x_dates_df = pd.DataFrame([test_x_dates], columns=[f"seq_day_{i+1}" for i in range(len(test_x_dates))])

        # Save corresponding dates for train/test labels
        train_y_dates_df = pd.DataFrame(train_y_dates, columns=[f"label_day_{i+1}" for i in range(len(train_y_dates[0]))])
        test_y_dates_df = pd.DataFrame([test_y_dates], columns=[f"label_day_{i+1}" for i in range(len(test_y_dates))])
        ...
        print("All train/test data successfully saved to Excel files.")

    except Exception as e:
        print(f"Error saving train/test data to Excel: {e}")
        raise
